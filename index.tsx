...
export default function Index() {
  ...
  const loadPage = async (page: number, refresh = false) => {
    ...
    try {
      const response = await api.gallery(page, controller.signal);
      if (generation !== generationRef.current) return;
      setItems((current) => {
        const incoming = page === 1 ? response.items : [...current, ...response.items];
        const unique = Array.from(new Map(incoming.map((item) => [item.id, item])).values());
        writeJson(CACHE_KEY, unique.slice(0, 90));
        return unique;
      });
      pageRef.current = page;
      setHasMore(response.has_more);
    } catch (error) {
      if ((error as Error).name !== "AbortError") setGalleryError(items.length ? "You’re offline. Cached images are still available." : "The gallery could not load right now.");
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      const [tokens, cached, saved, localProfile] = await Promise.all([
        storage.secureGet<Tokens | null>(TOKEN_KEY, null), readJson<GalleryItem[]>(CACHE_KEY, []), readJson<GalleryItem[]>(FAVORITES_KEY, []), readJson<Profile>(PROFILE_KEY, { name: "", bio: "" }),
      ]);
      if (!alive) return;
      setItems(cached); setFavorites(saved); setProfile(localProfile);
      if (!tokens) { setBooting(false); return; }
      try { const restored = await api.me(); if (!alive) return; setUser(restored); setProfile((current) => ({ ...current, name: current.name || restored.name })); setBooting(false); void loadPage(1); } catch { await storage.secureRemove(TOKEN_KEY); if (alive) setBooting(false); }
    })();
    return () => { alive = false; abortRef.current?.abort(); };
  }, []);

  const onAuth = async (values: { name: string; email: string; password: string }) => {
    setAuthError(""); setAuthBusy(true);
    try {
      if (mode === "forgot") { await api.forgotPassword(values.email); setAuthError("If an account exists, a reset link is on its way."); setMode("login"); }
      else { const response = mode === "register" ? await api.register(values.name, values.email, values.password) : await api.login(values.email, values.password); await storage.secureSet(TOKEN_KEY, { access_token: response.access_token, refresh_token: response.refresh_token }); setUser(response.user); setProfile((current) => ({ ...current, name: current.name || response.user.name })); void loadPage(1); }
    } catch (error) { setAuthError((error as Error).message); } finally { setAuthBusy(false); }
  };
